...
function signup($email, $pwdmd5, $name)
{
	//Check if email has been used
	if (hasRecordOf(DB_TABLE_USER, DB_USER_EMAIL, $email))
	{
		return "email_has_been_used";
	}
	else
	{
		$record[DB_USER_UUID] = guid();
		$record[DB_USER_EMAIL] = $email;
		$record[DB_USER_PWDMD5] = $pwdmd5;
		$record[DB_USER_NAME] = $name;
		insertRecord(DB_TABLE_USER, $record);

		return signin($email, $pwdmd5);
	}
}

function signin($email, $pwdmd5)
{
	//Check email
	if (!hasRecordOf(DB_TABLE_USER, DB_USER_EMAIL, $email))
	{
	...
	if (!hasRecordOfTwoConds(DB_TABLE_USER, DB_USER_EMAIL, $email, DB_USER_PWDMD5, $pwdmd5))
	{
		return "wrong_pwd";
	}
	$resultData = getuserinfobyemail($email);

	$record[DB_SESSION_UUID] = guid();
	$record[DB_SESSION_USERUUID] = $resultData[DB_USER_UUID];
	insertRecord(DB_TABLE_SESSION, $record);

	$resultData['sessionid'] = $record[DB_SESSION_UUID];

	return $resultData;
}

function signout($session_uuid)
{
	if (issignnedin($session_uuid))
	{
		$sql = "update " . DB_TABLE_SESSION . " set " . wrapCol(DB_SESSION_SIGNOUTTIME) . "=now() where " . wrapCol(DB_SESSION_UUID) . "=" . wrapStr($session_uuid);
		mysql_query($sql);
	}
}

function issignnedin($session_uuid)
{
	$sql = "select * from " . DB_TABLE_SESSION . " where " . wrapCol(DB_SESSION_UUID) . "=" . wrapStr($session_uuid) . " and " . wrapCol(DB_SESSION_SIGNOUTTIME) . " is null";;
...
{
	$sql = "select * from " . DB_TABLE_USER . " where " . wrapCol(DB_USER_EMAIL) . "=" . wrapStr($email);
	$result = mysql_query($sql);
	if(mysql_num_rows($result) <= 0)
	{
		return NULL;
...
function getuserinfobyuserid($user_uuid)
{
	$sql = "select * from " . DB_TABLE_USER . " where " . wrapCol(DB_USER_UUID) . "=" . wrapStr($user_uuid);
	$result = mysql_query($sql);
	if(mysql_num_rows($result) <= 0)
	{
		return NULL;
...
function getuseridbysessionid($session_uuid)
{
	$sql = "select * from " . DB_TABLE_SESSION . " where " . wrapCol(DB_SESSION_UUID) . "=" . wrapStr($session_uuid);
	$result = mysql_query($sql);
	$uid = NULL;
	while($row = mysql_fetch_array($result))
	{
		$uid = $row[DB_SESSION_USERUUID];
		break;
	}
	return $uid;
}

function addeventlog($session_uuid, $log_uuid, $event_uuid, $location, $comments)
{
	$user_uuid = getuseridbysessionid($session_uuid);
	$record[DB_EVENTLOG_UUID] = $log_uuid;
	$record[DB_EVENTLOG_USERUUID] = $user_uuid;
	$record[DB_EVENTLOG_EVENTUUID] = $event_uuid;
	$record[DB_EVENTLOG_LOCATION] = $location;
	$record[DB_EVENTLOG_COMMENTS] = $comments;
	insertRecord(DB_TABLE_EVENTLOG, $record);

	return getusereventlogbylogid($log_uuid);
}

function getusereventlogbylogid($log_uuid)
{
	$sql = "select * from " . DB_TABLE_EVENTLOG . " where " . wrapCol(DB_EVENTLOG_UUID) . "=" . wrapStr($log_uuid);
	$result = mysql_query($sql);
...
function updateeventlog($session_uuid, $log_uuid, $event_uuid, $location, $comments)
{
	$user_uuid = getuseridbysessionid($session_uuid);
	$conds[DB_EVENTLOG_UUID] = $log_uuid;

	$record[DB_EVENTLOG_EVENTUUID] = $event_uuid;
	$record[DB_EVENTLOG_LOCATION] = $location;
	$record[DB_EVENTLOG_COMMENTS] = $comments;

	updateRecord(DB_TABLE_EVENTLOG, $conds, $record);
	return getusereventlogbylogid($log_uuid);
}

function deleteeventlog($log_uuid)
{
	$sql = "delete from " . DB_TABLE_EVENTLOG . " where " . wrapCol(DB_EVENTLOG_UUID) . "=" . wrapStr($log_uuid);
	mysql_query($sql);
}

function geteventlogbysessionid($session_uuid)
{
	$user_uuid = getuseridbysessionid($session_uuid);
	$sql = "select * from " . DB_TABLE_EVENTLOG . " where " . wrapCol(DB_EVENTLOG_USERUUID) . "=" . wrapStr($user_uuid);
	$result = mysql_query($sql);
	return queryResultToArray($result);
}

//================below are legacy code (clean up later)=================
...
function getUserIdBySessionX($sessionId)
{
...
function getUserInfoByUserIdX($uid)
{
